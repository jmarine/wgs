...
public class OpenIdConnectClient implements Serializable
{
    ...
    public List<User> getGoogleFriends(User usr) throws Exception
    {
    ...
    public List<User> getFacebookFriends(User usr) throws Exception
    {
        EntityManager manager = Storage.getEntityManager();
        List<User> friends = usr.getFriends();
        ...
        for(int index = 0; index < items.size(); index++) {
            ...
            UserId friendId = new UserId(provider.getDomain(), id);
            User friend = manager.find(User.class, friendId);
            if(friend == null) {
                friend = new User();
                friend.setId(friendId);
