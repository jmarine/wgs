...
public class Module extends WampModule 
{
    ...
    {   
        ...
        for(Group g : query.getResultList()) {
            int refCount = 0;
            for(Member m : g.getMembers()) {
                if(m.getUser().equals(socket.getUserPrincipal())) {
            ...
            broadcastAppEventInfo(socket, g, (refCount == 0)? "group_deleted" : "group_updated");
        }
    ...
    private void broadcastAppEventInfo(WampSocket socket, Group g, String cmd) throws Exception
    {
        ...
        WampBroker.publishEvent(socket.getRealm(), WampProtocol.newId(), WampBroker.getTopic(getFQtopicURI("apps_event")), null, event, eligible, null, null, null, null, null);
